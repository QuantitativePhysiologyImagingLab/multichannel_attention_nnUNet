...
class AttachB0DirFromProps(BasicTransform):
    ...
    converts B0_world=[0,0,1] into voxel axes and adds it as data_dict['b0_dir'] (np.float32, (3,))
    """
    ...
    def _compute_b0_voxel(self, direction_tuple):
        R = np.array(direction_tuple, dtype=np.float32).reshape(3, 3)
        b0_world = np.array([0., 0., 1.], dtype=np.float32)
        # voxel = R^T * world  (your convention)
        b0_voxel = R.T @ b0_world
        n = np.linalg.norm(b0_voxel) + 1e-8
...
def _rotmat_xyz(rx, ry, rz):
    ...
    # match SpatialTransform default axis order: z, y, x (same as applying Rx then Ry then Rz to coords)
    # For images, batchgenerators applies rotations to the grid; using R = Rz @ Ry @ Rx is the usual convention.
    return (Rz @ Ry @ Rx).astype(np.float32)

class SpatialTransformWithB0(SpatialTransform):
    """
    Behaves like SpatialTransform but also rotates/mirrors data_dict['b0_dir'] with the SAME augmentation.
    """
    def __call__(self, **data_dict):
        # 1) Let parent compute its random params and apply to data/seg
        result = super().__call__(**data_dict)

        # 2) Retrieve the actually sampled rotation angles.
        # batchgenerators v2 stores the last-sampled params on the transform instance:
        # self.last_rotate_x, self.last_rotate_y, self.last_rotate_z in radians (present when p_rotation triggers)
        rx = getattr(self, 'last_rotate_x', 0.0) or 0.0
        ry = getattr(self, 'last_rotate_y', 0.0) or 0.0
        rz = getattr(self, 'last_rotate_z', 0.0) or 0.0
        R = _rotmat_xyz(rx, ry, rz)  # 3x3

        # 3) Mirror: parent sets self.last_mirror_axes to tuple of axes mirrored (e.g., (0,2))
        # In voxel axes (x=0,y=1,z=2) create diag(+/-1)
        M = np.eye(3, dtype=np.float32)
        if hasattr(self, 'last_mirror_axes') and self.last_mirror_axes is not None:
            for ax in self.last_mirror_axes:
                M[ax, ax] = -1.0

        A = (M @ R).astype(np.float32)  # total 3x3

        # 4) Rotate/mirror b0_dir if present
        if 'b0_dir' in result and result['b0_dir'] is not None:
            v = np.asarray(result['b0_dir'], dtype=np.float32)  # (3,)
            v2 = A @ v
            n = np.linalg.norm(v2) + 1e-8
            result['b0_dir'] = (v2 / n).astype(np.float32)

        return result
    
class EnsureB0Shape(BasicTransform):
    def __call__(self, **data_dict):
        if 'b0_dir' in data_dict and data_dict['b0_dir'] is not None:
            b0 = np.asarray(data_dict['b0_dir'], dtype=np.float32).reshape(3)
            data_dict['b0_dir'] = b0
        return data_dict

class nnUNetTrainerFrangiPhysicsWithAttentionFineTune(nnUNetTrainer):
    ...
    def _compute_B0_from_direction(self, dir_tuple):
        """
        dir_tuple: length-9 tuple from props['sitk_stuff']['direction'] (row-major 3x3)
        Returns torch.FloatTensor shape (3,) unit vector in voxel coords.
        """
        import numpy as np
        R = torch.tensor(np.array(dir_tuple).reshape(3, 3), dtype=torch.float32, device=self.device)
        B0_world = torch.tensor([0., 0., 1.], dtype=torch.float32, device=self.device)
        # voxel = R^T * world
        B0_voxel = R.t().matmul(B0_world)
        return B0_voxel / (B0_voxel.norm() + 1e-8)
    
    def _get_case_B0(self, case_key: str):
        """
        case_key: e.g. '3TA3007_008_V1' or whatever your identifiers look like
        Looks for <preprocessed_folder>/<case_key>.pkl and reads sitk_stuff.direction.
        Caches result on first use.
        """
        if case_key in self._b0_cache:
            return self._b0_cache[case_key]

        import pickle, os
        pkl_path = join(self.preprocessed_dataset_folder, f"{case_key}.pkl")
        with open(pkl_path, 'rb') as f:
            props = pickle.load(f)

        # per your printout, this file has `sitk_stuff` with 'direction'
    ...
    def get_dataloaders(self):
        ...
        tr_transforms = self.get_training_transforms(
            ...
            ignore_label=self.label_manager.ignore_label, preprocessed_dataset_folder=self.preprocessed_dataset_folder)
        ...
        val_transforms = self.get_validation_transforms(deep_supervision_scales,
                                                        ...
                                                        ignore_label=self.label_manager.ignore_label,
                                                        preprocessed_dataset_folder=self.preprocessed_dataset_folder)
